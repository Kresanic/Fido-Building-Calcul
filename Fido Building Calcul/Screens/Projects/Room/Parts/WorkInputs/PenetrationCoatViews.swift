...
struct PenetrationCoatingViews: View {
    ...
    var body: some View {
        ...
        if let room = fetchedRoom.first {
            ...
            VStack {
                ...
                if !room.associatedPenetrationCoatings.isEmpty {
                    ForEach(room.associatedPenetrationCoatings) { loopedObject in
                        
                        let objectCount = room.associatedPenetrationCoatings.count - (room.associatedPenetrationCoatings.firstIndex(of: loopedObject) ?? 0)
                        
                        PenetrationCoatingEditor(penetrationCoat: loopedObject, objectCount: objectCount)
                ...
                .clipShape(RoundedRectangle(cornerRadius: room.associatedPenetrationCoatings.isEmpty ? 20 : 30, style: .continuous))
        }
    }
    
    func createNewEntity(to room: Room) {
        withAnimation { scrollProxy.scrollTo(PenetrationCoating.scrollID, anchor: .top) }
        let newEntity = PenetrationCoating(context: viewContext)
...
fileprivate struct PenetrationCoatingEditor: View {
    
    @FetchRequest var fetchedEntities: FetchedResults<PenetrationCoating>
    @State var width = ""
    @State var height = ""
    @Environment(\.managedObjectContext) var viewContext
    ...
    init(penetrationCoat: PenetrationCoating, objectCount: Int) {
        
        self.objectCount = objectCount
        
        let entityRequest = PenetrationCoating.fetchRequest()
        
        let cUUID = penetrationCoat.cId ?? UUID()
        
        entityRequest.sortDescriptors = [NSSortDescriptor(keyPath: \PenetrationCoating.size1, ascending: true)]
    ...
    private func deleteObject(toDelete: PenetrationCoating) {
        
        if focusedDimension != nil { focusedDimension = nil }
        
        let requestObjectsToDelete = PenetrationCoating.fetchRequest()
        
        requestObjectsToDelete.sortDescriptors = [NSSortDescriptor(keyPath: \PenetrationCoating.size2, ascending: true)]
