    ...
    var body: some View {
        ...
        if let room = fetchedRoom.first {
            ...
            VStack {
                ...
                if !room.associatedLevellings.isEmpty {
                    ForEach(room.associatedLevellings) { loopedObject in
                        
                        let objectCount = room.associatedLevellings.count - (room.associatedLevellings.firstIndex(of: loopedObject) ?? 0)
                        
                        LevellingEditor(levelling: loopedObject, objectCount: objectCount)
                ...
                .clipShape(RoundedRectangle(cornerRadius: room.associatedLevellings.isEmpty ? 20 : 30, style: .continuous))
        }
    }
    
    func createNewEntity(to room: Room) {
        withAnimation { scrollProxy.scrollTo(Levelling.scrollID, anchor: .top) }
        let newEntity = Levelling(context: viewContext)
...
fileprivate struct LevellingEditor: View {
    
    @FetchRequest var fetchedEntities: FetchedResults<Levelling>
    @State var width = ""
    @State var length = ""
    @Environment(\.managedObjectContext) var viewContext
    ...
    init(levelling: Levelling, objectCount: Int) {
        
        self.objectCount = objectCount
        
        let entityRequest = Levelling.fetchRequest()
        
        let cUUID = levelling.cId ?? UUID()
        
        entityRequest.sortDescriptors = [NSSortDescriptor(keyPath: \Levelling.size1, ascending: true)]
    ...
    private func deleteObject(toDelete: Levelling) {
        
        if focusedDimension != nil { focusedDimension = nil }
        
        let requestObjectsToDelete = Levelling.fetchRequest()
        
        requestObjectsToDelete.sortDescriptors = [NSSortDescriptor(keyPath: \Levelling.size2, ascending: true)]
