...
struct PaintingCeilingViews: View {
            ...
            VStack {
                ...
                HStack {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        Text(PaintingCeiling.title)
                            .font(.system(size: 22, weight: .semibold))
                            .foregroundStyle(Color.brandBlack)
                        
                        Text(PaintingCeiling.subTitle)
                ...
                if !room.associatedPaintingCeilings.isEmpty {
                    ForEach(room.associatedPaintingCeilings) { loopedObject in
                        
                        let objectCount = room.associatedPaintingCeilings.count - (room.associatedPaintingCeilings.firstIndex(of: loopedObject) ?? 0)
                        
                        PaintingCeilingEditor(paintingCeiling: loopedObject, objectCount: objectCount)
                ...
                .clipShape(RoundedRectangle(cornerRadius: room.associatedPaintingCeilings.isEmpty ? 20 : 30, style: .continuous))
        }
    }
    
    func createNewEntity(to room: Room) {
        withAnimation { scrollProxy.scrollTo(PaintingCeiling.scrollID, anchor: .top) }
        let newEntity = PaintingCeiling(context: viewContext)
...
fileprivate struct PaintingCeilingEditor: View {
    
    @FetchRequest var fetchedEntities: FetchedResults<PaintingCeiling>
    @State var width = ""
    @State var length = ""
    @Environment(\.managedObjectContext) var viewContext
    ...
    init(paintingCeiling: PaintingCeiling, objectCount: Int) {
        
        self.objectCount = objectCount
        
        let entityRequest = PaintingCeiling.fetchRequest()
        
        let cUUID = paintingCeiling.cId ?? UUID()
        
        entityRequest.sortDescriptors = [NSSortDescriptor(keyPath: \PaintingCeiling.size1, ascending: true)]
    ...
    private func deleteObject(toDelete: PaintingCeiling) {
        
        if focusedDimension != nil { focusedDimension = nil }
        
        let requestObjectsToDelete = PaintingCeiling.fetchRequest()
        
        requestObjectsToDelete.sortDescriptors = [NSSortDescriptor(keyPath: \PaintingCeiling.size1, ascending: true)]
