    ...
    var body: some View {
        ...
        if let room = fetchedRoom.first {
            ...
            VStack {
                ...
                if !room.associatedPlasteringCeiling.isEmpty {
                    ForEach(room.associatedPlasteringCeiling) { loopedObject in
                        
                        let objectCount = room.associatedPlasteringCeiling.count - (room.associatedPlasteringCeiling.firstIndex(of: loopedObject) ?? 0)
                ...
                .clipShape(RoundedRectangle(cornerRadius: room.associatedPlasteringCeiling.isEmpty ? 20 : 30, style: .continuous))
        }
    }
...
fileprivate struct PlasteringCeilingEditor: View {
    ...
    init(plasteringCeiling: PlasteringCeiling, objectCount: Int) {
        ...
        entityRequest.sortDescriptors = [NSSortDescriptor(keyPath: \PlasteringCeiling.width, ascending: true)]
    ...
    private func deleteObject(toDelete: PlasteringCeiling) {
        ...
        requestObjectsToDelete.sortDescriptors = [NSSortDescriptor(keyPath: \PlasteringCeiling.length, ascending: true)]
